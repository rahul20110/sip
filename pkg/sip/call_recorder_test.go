...
func TestRecTruthyAndGating(t *testing.T) {
	require.True(t, recTruthy("true"))
	require.True(t, recTruthy("1"))
	require.True(t, recTruthy(" YES "))
	require.True(t, recTruthy("on"))
	require.False(t, recTruthy(""))
	require.False(t, recTruthy("false"))
	require.False(t, recTruthy("maybe"))

	require.True(t, trunkWantsRecording(map[string]string{"X-Lk-Record": "true"}))
	require.True(t, trunkWantsRecording(map[string]string{"x-lk-record": "TRUE"})) // case-insensitive key+value
	require.False(t, trunkWantsRecording(map[string]string{"X-Lk-Record": "false"}))
	require.False(t, trunkWantsRecording(map[string]string{"X-Other": "true"}))
	require.False(t, trunkWantsRecording(nil))

	t.Setenv(recEnabledEnv, "")
	require.False(t, recordingEnabled())
	t.Setenv(recEnabledEnv, "true")
	require.True(t, recordingEnabled())
}
...
func TestCallRecorderWAVChannels(t *testing.T) {
	...
	rec := newCallRecorder(logger.GetLogger(), "test-call", "trunk-test")
	rec.Arm()
	...
	path := filepath.Join(dir, "test-call.wav")
...
func TestCallRecorderUnansweredNoFile(t *testing.T) {
	...
	rec := newCallRecorder(logger.GetLogger(), "never-answered", "trunk-test")
...
func TestCallRecorderDriftSoak(t *testing.T) {
	...
	rec := newCallRecorder(logger.GetLogger(), "drift-soak", "trunk-test")
	...
	left, right := readWAV(t, filepath.Join(dir, "drift-soak.wav"))
...
func TestCallRecorderConcurrentLoad(t *testing.T) {
	...
	for ri := 0; ri < nRecorders; ri++ {
		rec := newCallRecorder(logger.GetLogger(), fmt.Sprintf("load-%d", ri), "trunk-test")
		recs[ri] = rec
		rec.Arm()
		wg.Add(1)
		go func() {
	...
	for ri, rec := range recs {
		left, right := readWAV(t, filepath.Join(dir, fmt.Sprintf("load-%d.wav", ri)))
		var lReal, rReal int
		for _, v := range left {
			if v != 0 {
...
func TestCallRecorderPreAnswerDiscardedPostAnswerKept(t *testing.T) {
	...
	rec := newCallRecorder(logger.GetLogger(), "answered-later", "trunk-test")
	frame := make(msdk.PCM16Sample, recFrameSamples)
	for i := range frame {
		frame[i] = 777
	}
	...
	left, _ := readWAV(t, filepath.Join(dir, "answered-later.wav"))
	var got int
	for _, v := range left {
		if v == 777 {
			got++
