...
//  3. early media disabled by env, carrier still sends 183
//     (early media NOT set up; rec created post-dialSIP)
//
// PRODUCTION NOTE: our 183 early media is gated by the SIP_ENABLE_EARLY_MEDIA
// env var (default OFF), not a per-call attribute — so each scenario sets the
// env explicitly rather than passing an AttrSIPEarlyMedia attribute.
...
// Scenario 1: early media enabled (env on), carrier sends 183+SDP.
...
// Scenario 3: early media DISABLED by env, carrier still sends 183+SDP.
// The 183's SDP must be ignored (no early media) and the recorder must
// still arm at answer via the non-early path.
func TestRecArmEarlyMediaDisabledWith183(t *testing.T) {
	t.Setenv("SIP_ENABLE_EARLY_MEDIA", "false")
	withFakeRecPool(t)
	call := runRecArmCall(t, "arm-em-off-183", nil, true)
